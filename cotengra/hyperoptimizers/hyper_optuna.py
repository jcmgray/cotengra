...
class OptunaOptLib(HyperOptLib):
    """Hyper-optimization using ``optuna``."""
    ...
    def get_setting(self):
        with warnings.catch_warnings():
            warnings.filterwarnings(
                action="ignore", message=".*divide by zero.*"
            )
            warnings.filterwarnings(
                action="ignore",
    ...
    def report_result(self, setting, trial, score):
        self._study.tell(setting["trial_number"], score)


register_hyper_optlib("optuna", OptunaOptLib)
