...
This nested parallelism is enabled by using [`dask.distributed`](https://distributed.dask.org/en/latest/) which should also hopefully allow scaling up the whole process to large clusters. You can also use [`ray`](https://docs.ray.io/) as an potentially more performant alternative.
...
* `'ray'` : use (and if needed initialize) ``ray`` for parallelism
